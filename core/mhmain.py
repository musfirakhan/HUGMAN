...
class MHApplication(gui3d.Application, mh.Application):
    ...
    def grabScreen(self):
        ...
        if hasattr(self.mainwin, 'canvas') and self.mainwin.canvas:
            self.mainwin.canvas.makeCurrent()
        ...
        try:
            # Try OpenGL method first
            # Ensure viewport is set correctly for the entire window
            if hasattr(self.mainwin, 'canvas') and self.mainwin.canvas:
                self.mainwin.canvas.makeCurrent()
            ...
            mh.grabScreen(0, 0, G.windowWidth, G.windowHeight, filename, productionRender=False)
            self.status("Screengrab saved to %s", filename)
        except Exception as e:
            # Fallback to Qt's native screen capture
            try:
                from PyQt5.QtWidgets import QApplication
                from PyQt5.QtCore import QRect
                
                # Capture only the 3D canvas area, not the entire window
                if hasattr(self.mainwin, 'canvas') and self.mainwin.canvas:
                    # Get the canvas geometry relative to the main window
                    canvas_geometry = self.mainwin.canvas.geometry()
                    window_geometry = self.mainwin.geometry()
                    
                    # Calculate the absolute position of the canvas on screen
                    abs_x = window_geometry.x() + canvas_geometry.x()
                    abs_y = window_geometry.y() + canvas_geometry.y()
                    
                    screen = QApplication.primaryScreen()
                    pixmap = screen.grabWindow(self.mainwin.winId(), 
                                             canvas_geometry.x(), canvas_geometry.y(), 
                                             canvas_geometry.width(), canvas_geometry.height())
                else:
                    # Fallback to window capture if canvas not available
                    geometry = self.mainwin.geometry()
                    screen = QApplication.primaryScreen()
                    pixmap = screen.grabWindow(self.mainwin.winId(), 
                ...
                self.status("Screengrab saved to %s (Qt method)", filename)
            except Exception as e2:
                self.status("Failed to save screengrab: %s", str(e2))
